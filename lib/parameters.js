...
parameters.fromProperties = function(schemaObj, parameterType) {
  ...
  if (schemaObj.properties === undefined) {
    ...
    keys.forEach((element, index) => {
      ...
      // "example" is not specified as valid in Swagger but
      // "x-example" is actually supported and will prefill values
      // in Swagger UI. This field is also useful for our homemade doc.
      if (['header', 'query', 'path'].includes(parameterType) && item['example']) {
        item['x-example'] = item['example'];
        delete item['example']
      }
